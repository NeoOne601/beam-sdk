# Ajna SDK — Wave 1 Parallel Agent Prompts
...
**Dependency note for Agent D:** The TypeScript method `getScanResult()` calls `ajna_session_get_result_json` which Agent B creates. Write the TypeScript as if that symbol exists — do not attempt a WASM build.
...
<context>
Repository: ajna-sdk — cross-platform identity document scanning SDK.
Language stack: Rust core + C++ ML bridges + Swift/Kotlin platform adapters.

Security remediation VR-1 added 4 nonce-binding parameters to ajna_session_push_result.
The canonical C declaration in include/ajna_ffi.h now requires 13 parameters total.
...
Three C++ bridge files still call the OLD signature (8–9 args) — C ABI violation causing undefined behaviour on every inference call. include/ajna_ffi.h is already correct. core/src/ffi.rs is already correct.
</context>
...
<task>
Fix every call site of ajna_session_push_result to pass 13 arguments by inserting the 4 null/zero nonce-binding parameters at positions 8–11.
...
FILE 1: platform/ios/coreml_bridge.mm
  There are exactly 2 calls to ajna_session_push_result in this file.
  ...
  Call site 2: after the fields[] loop at the end of ajna_coreml_process().
    Preserve existing overall_conf variable and `/*include_pqc_sig=*/ true`.

FILE 2: platform/android/tflite_bridge.cpp
  There is exactly 1 call to ajna_session_push_result inside push_output_to_session().
...
FILE 3: platform/wasm/onnx_bridge.cpp
  There is exactly 1 call to ajna_session_push_result inside ajna_wasm_process_frame().
...
MUST NOT change any function signature, struct definition, class member, or logic other than the argument lists of ajna_session_push_result calls.
...
2. ✅ Report: "Located N calls to ajna_session_push_result in coreml_bridge.mm at lines [X, Y]"
...
6. ✅ Report: "Located 1 call to ajna_session_push_result in push_output_to_session at line [X]"
...
10. ✅ Report: "Located 1 call to ajna_session_push_result in ajna_wasm_process_frame at line [X]"
11. Apply fix. Output the complete modified file.
12. ✅ Report: "onnx_bridge.cpp — 1 call site now passes 13 arguments"
</execution_steps>

<success_criteria>
Every call to ajna_session_push_result across all 3 files passes exactly 13 arguments.
No call site has 8, 9, 10, 11, or 12 arguments — those counts mean the fix is incomplete.
Static check (run by human post-wave): grep for ajna_session_push_result in the 3 files and count commas — each call must have 12 commas between its arguments.
</success_criteria>
```
...
Current broken state:
  ...
  - platform/ios/AjnaSDK.swift ignores Rust data entirely and returns: AjnaScanResult(documentType: "document", issuingCountry: "UNK", confidence: 0.92)
...
<task_part_1_ffi_rs>
Add a new public FFI function ajna_session_get_result_json to core/src/ffi.rs.

Insert it AFTER the ajna_session_push_result function block and BEFORE any #[cfg(test)] block.
...
Safety comment (MUST include):
  // Safety: handle must be a valid AjnaSessionHandle produced by ajna_session_create,
...
Add the function to the pub use re-exports in core/src/lib.rs:
  pub use ffi::ajna_session_get_result_json;
</task_part_1_ffi_rs>

<task_part_2_ajna_ffi_h>
Add the C declaration for ajna_session_get_result_json to include/ajna_ffi.h.

Insert it AFTER the ajna_session_push_result declaration, inside the existing extern "C" block.
...
<task_part_4_ajnasdk_swift>
Edit platform/ios/AjnaSDK.swift.

STEP A — Add bridge declaration.
Find the block of @_silgen_name declarations near the top of the file (where ajna_session_get_state, ajna_session_create, etc. are declared). Add this declaration alongside them:
...
<execution_steps>
1. Read core/src/ffi.rs — find end of ajna_session_push_result and location of #[cfg(test)]
2. ✅ Report: "Insertion point in ffi.rs: after line [X], before #[cfg(test)] at line [Y]"
3. Add ajna_session_get_result_json. Output complete modified core/src/ffi.rs.
4. ✅ Report: "ffi.rs updated — ajna_session_get_result_json added"
5. Read core/src/lib.rs — check pub use ffi:: exports
6. Add ajna_session_get_result_json to pub use. Output complete modified core/src/lib.rs.
7. ✅ Report: "lib.rs updated — function added to pub use exports"
8. Read include/ajna_ffi.h — find ajna_session_push_result declaration
9. Add C declaration. Output complete modified include/ajna_ffi.h.
10. ✅ Report: "ajna_ffi.h updated — C declaration added at line [X]"
...
14. Read platform/ios/AjnaSDK.swift — locate @_silgen_name block and if state == 3 block
15. Apply both changes. Output complete modified platform/ios/AjnaSDK.swift.
16. ✅ Report: "AjnaSDK.swift updated — FFI declaration added, state==3 block now calls ajna_session_get_result_json"
</execution_steps>

<success_criteria>
PASS: grep "ajna_session_get_result_json" core/src/ffi.rs returns the function definition.
PASS: grep "ajna_session_get_result_json" include/ajna_ffi.h returns the C declaration.
PASS: grep "serde_json" core/Cargo.toml returns 1 line under [dependencies].
FAIL condition: The string literal "issuingCountry: \"UNK\"" appears as a NON-fallback primary result in AjnaSDK.swift. The fallback branch (inside the else block with NSLog) is acceptable; the primary path MUST NOT hardcode it.
</success_criteria>
```

---

## AGENT C — Android JNI Fix + AjnaScanner Public API
...
STOP AND ASK before deleting any file. The rename of AjnaNativeBridge.kt is handled by creating a new file (AjnaSDK.kt) and leaving a deletion note — do not silently delete files without confirmation.
</critical_constraint>
...
BUG 1 — JNI class name mismatch (runtime crash):
  tflite_bridge.cpp exports JNI symbols under class path com.ajna.sdk.AjnaSDK:
    Java_ai_ajna_ajna_AjnaSDK_nativeCreateInferenceEngine
    Java_ai_ajna_ajna_AjnaSDK_nativeDestroyInferenceEngine
    Java_ai_ajna_ajna_AjnaSDK_nativeOnFrame  (etc.)
  But platform/android/AjnaNativeBridge.kt declares these externals in class AjnaNativeBridge.
  Result: UnsatisfiedLinkError at runtime on every Android device.
  Fix: The Kotlin class must be named AjnaSDK in package com.ajna.sdk.
...
BUG 2 — Missing handle propagation (compile failure):
  AjnaCameraAdapter.kt calls nativeBridge.onFrame(yBuffer, uvBuffer, width, height, ...)
  But AjnaNativeBridge.kt's onFrame wrapper requires engineHandle: Long and sessionHandle: Long as first two parameters.
  The adapter has no access to these handles and cannot pass them.
  Fix: After the rename, AjnaSDK owns engineHandle and sessionHandle as instance state.
  AjnaCameraAdapter receives a AjnaSDK reference and calls onFrame() with no handle parameters
  (the handles are encapsulated inside AjnaSDK).

MISSING FEATURE — Android has no high-level developer-facing API:
  iOS has AjnaScanner (configure/startScan/stopScan). Android exposes raw JNI handles.
  Fix: Create AjnaScanner.kt as the public Android API mirroring iOS parity.
</context>

<scope_lock>
MUST create or modify ONLY these files:
  CREATE:   platform/android/AjnaSDK.kt              (new — replaces AjnaNativeBridge.kt)
  MODIFY:   platform/android/AjnaCameraAdapter.kt    (update reference from AjnaNativeBridge → AjnaSDK)
  CREATE:   platform/android/AjnaScanner.kt          (new high-level public API)

MUST NOT modify:
  platform/android/tflite_bridge.cpp   (JNI symbols are already correct)
  Any iOS, WASM, Rust, or backend file

NOTE ON AjnaNativeBridge.kt: After creating AjnaSDK.kt, add a comment block at the top of
AjnaNativeBridge.kt saying:
  // DEPRECATED: This file is superseded by AjnaSDK.kt.
...
<task_file_1_ajnasdk_kt>
Create platform/android/AjnaSDK.kt with this exact content and structure:

Package: com.ajna.sdk

Class name: AjnaSDK (MUST match the JNI symbol prefix Java_ai_ajna_ajna_AjnaSDK_*)
...
Internal accessor (package-private visibility for AjnaScanner):
...
JNI external declarations (identical to AjnaNativeBridge.kt — all same signatures):
  Copy all external fun declarations verbatim from AjnaNativeBridge.kt.

Companion object:
  companion object {
      init { System.loadLibrary("ajna_sdk") }
  }

SessionState enum (copy from AjnaNativeBridge.kt verbatim):
  enum class SessionState { IDLE, SCANNING, INFERRING, COMPLETE, FAILED }
</task_file_1_ajnasdk_kt>

<task_file_2_ajnacameraadapter_kt>
Edit platform/android/AjnaCameraAdapter.kt.

Change 1: Constructor parameter
  FROM: private val nativeBridge: AjnaNativeBridge
  TO:   private val ajnaSdk: AjnaSDK

Change 2: All references to nativeBridge inside the class body
  Replace every occurrence of nativeBridge. with ajnaSdk.
...
Change 3: The processFrame() call inside the OnImageAvailableListener lambda
  ...
  MUST NOT pass engineHandle or sessionHandle — AjnaSDK.onFrame() encapsulates them.

Make NO other changes to AjnaCameraAdapter.kt.
</task_file_2_ajnacameraadapter_kt>

<task_file_3_ajnascanner_kt>
Create platform/android/AjnaScanner.kt.

This is the developer-facing public API. It MUST mirror the iOS AjnaScanner pattern.
...
<deprecation_note>
After creating AjnaSDK.kt, open AjnaNativeBridge.kt and insert at the very top (before the package declaration):

  // DEPRECATED — superseded by AjnaSDK.kt (created in Wave 1 Agent C).
  // TODO: Delete this file after Wave 1 agent merge is confirmed.
  // Do NOT add new code here.

Do NOT delete any existing code in AjnaNativeBridge.kt beyond adding this comment.
</deprecation_note>

<execution_steps>
1. Read platform/android/AjnaNativeBridge.kt — extract all external fun declarations and SessionState enum
2. ✅ Report: "Read AjnaNativeBridge.kt — found [N] external declarations"
3. Create platform/android/AjnaSDK.kt with full content per task_file_1_ajnasdk_kt
4. ✅ Report: "AjnaSDK.kt created — class name matches JNI prefix Java_ai_ajna_ajna_AjnaSDK_*"
5. Add deprecation comment to AjnaNativeBridge.kt
6. ✅ Report: "AjnaNativeBridge.kt marked deprecated"
7. Read platform/android/AjnaCameraAdapter.kt
8. Apply the 3 changes (constructor, references, processFrame call)
9. Output complete modified AjnaCameraAdapter.kt
10. ✅ Report: "AjnaCameraAdapter.kt updated — uses AjnaSDK, no handle parameters in onFrame call"
11. Create platform/android/AjnaScanner.kt with full content per task_file_3_ajnascanner_kt
12. ✅ Report: "AjnaScanner.kt created — configure/startScan/stopScan public API"
</execution_steps>

<success_criteria>
PASS: grep "class AjnaSDK" platform/android/AjnaSDK.kt returns 1 result.
PASS: grep "Java_ai_ajna_ajna_AjnaSDK" platform/android/tflite_bridge.cpp still matches the class name in AjnaSDK.kt (com.ajna.sdk.AjnaSDK).
PASS: grep "engineHandle\|sessionHandle" platform/android/AjnaCameraAdapter.kt returns 0 results — handles MUST NOT appear in the adapter.
PASS: grep "nativeCreateInferenceEngine" platform/android/AjnaCameraAdapter.kt returns 0 results — raw JNI MUST NOT be called from the adapter.
PASS: grep "class AjnaScanner" platform/android/AjnaScanner.kt returns 1 result with startScan, stopScan, configure methods.
</success_criteria>
```

---

## AGENT D — Web TypeScript SDK Wrapper (@ajna/verify)
...
DEPENDENCY NOTE: The TypeScript method getScanResult() calls ajna_session_get_result_json which is being added to the Rust FFI by Agent B in parallel. Write the TypeScript as if that function exists in the compiled WASM binary. The TypeScript can be written and type-checked without the Rust symbol being present — do not let this dependency block you.
</dependency_note>
</critical_constraint>
...
Current broken state:
  - The WASM module (ajna_sdk.js / ajna_sdk.wasm) exposes low-level C functions via Emscripten.
  - A TypeScript declaration file at publishing/publish_npm.sh → ajna_verify.d.ts documents a high-level API (initAjnaVerify, processFrame, getScanResult, destroySession) but NO implementation file exists.
...
WASM memory architecture (mandatory copy — this is not a bug):
  JavaScript heap and WASM linear memory are isolated. Passing ImageData to ONNX Runtime requires:
    ...
    4. module._free(ptr)
  This copy MUST be encapsulated inside AjnaScanner.processFrame(). Callers MUST NOT touch _malloc or HEAPU8.

WASM function names available at runtime:
  ajna_wasm_create(modelPath: string): number
  ajna_wasm_process_frame(wasmSession, rgbaPtr, width, height, rustSession): void
  ...
  ajna_session_get_state(session: number): number    // 0=Idle,1=Scanning,2=Inferring,3=Complete,4=Failed
  ajna_session_get_result_json(session, outBuf, outBufLen): number  // Added by Agent B
...
MUST create or modify ONLY these 3 files:
  CREATE:  platform/web/AjnaScanner.ts
  MODIFY:  samples/web/main.ts
  MODIFY:  scripts/package_wasm_npm.sh

MUST NOT modify:
  publishing/publish_npm.sh
  publishing/ajna_verify.d.ts   (the existing .d.ts is already correct — do not touch it)
  core/src/ffi.rs
  Any other file
</scope_lock>

<task_file_1_ajnascanner_ts>
Create platform/web/AjnaScanner.ts.
...
CHANGE 1: Import AjnaScanner at the top of the file (after any existing imports):
  import { AjnaScanner } from '../../platform/web/AjnaScanner.js';
...
Replace its entire body with an implementation that:
  ...
  3. Creates a AjnaScanner instance and calls configure() with backendUrl and modelPath from loadSettings()
...
After the line that copies ajna_sdk.js and ajna_sdk.wasm to NPM_DIR, add:
...
After the package.json write block, change the "main" field from "ajna_sdk.js" to "AjnaScanner.js":
  "main": "AjnaScanner.js",

Add "AjnaScanner.ts" and "AjnaScanner.js" to the "files" array in package.json:
  "files": [
    "ajna_sdk.js",
    "ajna_sdk.wasm",
    "ajna-sdk.d.ts",
...
This is a comment only — do NOT actually invoke tsc (it will fail because ajna_sdk.js type declarations are not present at script run time).
</task_file_3_package_wasm_npm_sh>

<execution_steps>
1. Read publishing/publish_npm.sh to understand the ajna_verify.d.ts contract
2. ✅ Report: "Read publish_npm.sh — ajna_verify.d.ts documents [N] exported symbols"
...
5. Create platform/web/AjnaScanner.ts with full content
6. ✅ Report: "AjnaScanner.ts created — configure/processFrame/getScanResult/destroy methods implemented"
...
9. ✅ Report: "main.ts updated — delay() removed, real AjnaScanner integration wired"
10. Read scripts/package_wasm_npm.sh
11. Apply the 3 changes (copy AjnaScanner.ts, update main field, update files array, add tsc comment)
12. Output complete modified scripts/package_wasm_npm.sh
13. ✅ Report: "package_wasm_npm.sh updated — AjnaScanner.ts included in npm dist"
</execution_steps>

<success_criteria>
PASS: grep "MANDATORY COPY" platform/web/AjnaScanner.ts returns the comment in processFrame().
PASS: grep "_malloc\|_free\|HEAPU8" platform/web/AjnaScanner.ts — results exist ONLY inside AjnaScanner class methods, never in exported interfaces or the module-level scope.
PASS: grep "delay(" samples/web/main.ts returns 0 results.
PASS: grep "AjnaScanner" scripts/package_wasm_npm.sh returns at least 1 result in the files array.
FAIL condition: AjnaScanner.ts exports _malloc or HEAPU8 as public API — this breaks the encapsulation contract.
</success_criteria>
```
...
Purpose of this work:
  ...
  validates everything in the pipeline: quality gates → inference → ajna_session_push_result → Rust session → PQC signing → Complete state.
...
8. Add synthetic_test.tflite entry and /tmp/ajna_ffi_tests entry
9. Output complete modified .gitignore
10. ✅ Report: ".gitignore updated — synthetic_test.tflite and /tmp/ajna_ffi_tests excluded"
</execution_steps>
...
grep -c "ajna_session_push_result" platform/ios/coreml_bridge.mm         # expect 2
grep -c "ajna_session_get_result_json" core/src/ffi.rs                   # expect 1
grep -c "class AjnaSDK" platform/android/AjnaSDK.kt                      # expect 1
grep "MANDATORY COPY" platform/web/AjnaScanner.ts                        # expect 1 match
