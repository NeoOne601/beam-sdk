...
fuzz_target!(|data: &[u8]| {
    ...
    for _ in 0..num_fields {
        ...
        let key_len = (data[offset] as usize).min(64).min(data.len().saturating_sub(offset + 3));
        let val_len = (data[offset + 1] as usize).min(256).min(data.len().saturating_sub(offset + 2 + key_len));
