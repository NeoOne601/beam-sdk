# AJNA SDK — Architecture, Security, and Post-Quantum Threat Analysis
### A White Paper for the Engineering Leadership of Ajna AI
...
## 1. Company Intelligence — Ajna AI

**Ajna AI** is an early-stage startup building an intelligent identity verification and fraud detection platform. Their product acts as a "digital compliance officer," transforming complex fraud signals into actionable intelligence for businesses.

**Ajna** is their strategic infrastructure bet: rather than pay per-scan licensing fees to Scandit (reported: ~$0.03–$0.15 per scan) or Microblink, Ajna is building the full scanning stack in-house. The motivations are:
...
**Market context:** Ajna operates in the identity verification (IDV) space alongside Socure, Veriff, Sumsub, and Persona. The differentiator is on-device processing — competitors primarily rely on server-side document analysis, creating latency and data residency compliance complexity. On-device processing eliminates both.
...
Ajna processes identity documents — passports, national ID cards, driving licences. The security requirements are:
...
| Algorithm | FIPS | Type | Quantum Security | Use in Ajna |
|-----------|------|------|-----------------|-------------|
...
### 3.5 Key Storage — Current and Planned

The ML-DSA private key is currently held in mlock()-protected process memory on non-WASM targets. Hardware security element integration is on the Phase 2 roadmap:

**Current implementation (Phase 1):**
On iOS and Android, `PqcSigner::generate()` calls `libc::mlock()` on the private key allocation immediately after key generation. The key is zeroed via volatile write and `munlock()`-ed on `Drop`. This prevents swap to disk but does not protect against a compromised OS kernel.

**Phase 2 roadmap — iOS (Secure Enclave / SEP):**
The Secure Enclave Processor (SEP) is an isolated coprocessor with its own boot ROM and encrypted memory. In Phase 2, the ML-DSA private key will be generated inside the SEP and never leave. The SEP will sign a challenge; the application will receive only the signature. iOS 17+ supports custom key types in the SEP via the CryptoKit framework.

**Phase 2 roadmap — Android (StrongBox Keymaster):**
StrongBox is a dedicated security chip (Titan M on Pixel, TrustZone-based on Qualcomm/MediaTek) that provides hardware-backed key storage. In Phase 2, the Android Keystore System API (`KeyPairGenerator` with `StrongBoxBacked = true`) will ensure private key operations happen inside the security element. ML-DSA support will require Android 14+ with the `FEATURE_STRONGBOX_KEYSTORE` flag.
...
*Mitigations:*
- Liveness detection (blink, motion prompt) — Ajna should integrate a separate liveness model.
...
- Replace ML-DSA stub with production pqcrypto-dilithium 0.5 implementation (implementing the NIST standard).
- Integrate Secure Enclave (iOS) and StrongBox (Android) for hardware-backed private key storage (Phase 2 roadmap item).
...
| Dimension | Scandit | Microblink (BlinkID) | Ajna (proposed) |
|-----------|---------|---------------------|-----------------|
...
Ajna's architecture as described in the job specification is technically sound and achievable by a 2–3 person team in 6–8 months given the right expertise. The C++/Rust split is the right engineering decision for the problem.
...
3. **Store private keys with mlock()-protection now; migrate to hardware security elements in Phase 2.** Secure Enclave (iOS) and StrongBox (Android) integration is on the Phase 2 roadmap and is required for a compliance-grade IDV product.
