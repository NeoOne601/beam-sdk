...
// platform/android/AjnaNativeBridge.kt
// JNI bridge from Kotlin to the native Ajna SDK (tflite_bridge.cpp / libajna_sdk.so).
...
class AjnaNativeBridge {
    ...
    /**
     * Handle a frame delivered by [AjnaCameraAdapter].
     * This is the primary integration point — wire [AjnaCameraAdapter.delegate]
     * to call this method from [AjnaCameraAdapter.processFrame].
     *
    ...
    /**
     * Camera error callback — invoked by [AjnaCameraAdapter] on device failure.
     * Override or observe this in the host application.
     */
    open fun onCameraError(errorCode: Int) {
        android.util.Log.e("AjnaNativeBridge", "Camera error: $errorCode")
    }

    companion object {
        init {
            // Load the shared native library. The .so is named libajna_sdk.so.
