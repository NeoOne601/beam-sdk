...
// AVFoundation camera adapter for Beam SDK.
...
public protocol BeamFrameDelegate: AnyObject {
    func didReceiveFrame(_ buffer: CVPixelBuffer, timestamp: CMTime)
}

public final class BeamCameraAdapter: NSObject {

    private var captureSession:     AVCaptureSession?
    private var videoOutput:        AVCaptureVideoDataOutput?
    private let sessionQueue =      DispatchQueue(label: "ai.surt.beam.camera",
                                                  qos: .userInteractive)
    public weak var delegate:       BeamFrameDelegate?
    ...
    public func configure() throws {
        ...
        guard let device = bestBackCamera() else {
            throw BeamError.noCameraAvailable
        }
...
extension BeamCameraAdapter: AVCaptureVideoDataOutputSampleBufferDelegate {
