 ...
 *
 * Architecture note:
 *   Camera (Kotlin) → BeamCameraAdapter → Quality Gates (Rust via JNI)
 *   → TFLite inference (C++ GPU delegate) → beam_session_push_result (Rust FFI)
 *   → PQC signing (Rust) → ScanResult delivered to this activity
 */
class ScanActivity : AppCompatActivity() {

    // In production, these would be Beam SDK handles obtained via JNI
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // In production:
        // 1. Create BeamCameraAdapter with camera2 API
        // 2. Create Rust session via beam_session_create()
        // 3. Create quality gate via beam_gate_create()
        // 4. Start session via beam_session_start()
     ...
     * In production, this is driven by real camera frames and Beam SDK callbacks.
     */
    ...
    override fun onDestroy() {
        super.onDestroy()
        // In production: beam_session_destroy(sessionHandle)
        // In production: beam_gate_destroy(gateHandle)
    }
}
