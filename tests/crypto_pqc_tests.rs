...
mod tests {
    ...
    fn sign_level3_signature_length() {
        ...
        // Dilithium-3 detached signature: 3293 bytes (FIPS 204 Table 2)
        assert_eq!(
            sig.len(), 3293,
            "Dilithium-3 signature must be 3293 bytes, got {}",
            sig.len()
        );
    }
    ...
    fn private_key_not_in_signature_output() {
        ...
        assert_eq!(
            sig.len(), 3293,
            "Signature length must be 3293 (real Dilithium-3, not a stub zero vector)"
        );
